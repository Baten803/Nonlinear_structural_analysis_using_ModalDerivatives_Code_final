...
elseif DynamicOnly_on == 1
    for i = 1:nel
      if ismember(i,Tflat) 
        G(i,:) = [E_deck A_deck I_deck rho c_damp 0];
      elseif ismember(i,Tincleft1)
        G(i,:) = [E_cable A_cable I_cable rho c_damp 0];
      elseif ismember(i,Tincleft2)
        G(i,:) = [E_cable A_cable I_cable rho c_damp 0];
      elseif ismember(i,Tincright1)
        G(i,:) = [E_cable A_cable I_cable rho c_damp 0];
      elseif ismember(i,Tincright2)
        G(i,:) = [E_cable A_cable I_cable rho c_damp 0];
      elseif ismember(i,Tl)
        G(i,:) = [E_hanger A_hanger I_hanger rho c_damp 0];
      elseif ismember(i,Tv)
        G(i,:) = [E_hanger A_hanger I_hanger rho c_damp 0];
      elseif ismember(i,Th)
        G(i,:) = [E_hanger A_hanger I_hanger rho c_damp 0];
...
%% Solving the linearized eigenvalue problem including prestress
...
R_static = R(X,T,G,D,dL_static,bL,nd,nel);
R0_dynamic = R(X,T,G,D,dL_dynamic,bL,nd,nel);
Q0 = zeros(nd,1);
q0 = [-1 0 0 1 0 0]';
for el = 1:nel
  N0 = G(el,6);
...
t_shift = 0;
R_dynamic_phys = R0_dynamic*(F*sin(13.4911*(t_CDM_phys-t_shift)));
R_dynamic_tb = R0_dynamic*(F*sin(13.4911*(t_CDM_tb-t_shift)));
R_dynamic_phys(:,t_CDM_phys<t_shift) = 0;
R_dynamic_tb(:,t_CDM_tb<t_shift) = 0;

if Physical_on == 1 && DynamicOnly_on == 0
    R_load_phys = (R_static + R_dynamic_phys) - Q0;
    V_phys_pre = V_physical_pre(X,T,G,D,R_load_phys,U,nno,nel,nd,dt_phys,t_CDM_phys,V0,DV0,zeta,PHI_phys,size(phi,2),Omega);
    V_phys_mid = V_phys_pre(11*3-1,2:end);
elseif Physical_on == 1 && DynamicOnly_on == 1
    R_load_phys = R_dynamic_phys;
    V_phys = V_physical(X,T,G,D,R_load_phys,U,nno,nel,nd,dt_phys,t_CDM_phys,V0,DV0,zeta,PHI_phys,size(phi,2),Omega);
    V_phys_mid = V_phys(11*3-1,2:end);
elseif Physical_on == 0
    V_phys_mid = zeros(1,length(t_CDM_phys));
end

if Taylor_on == 1 && DynamicOnly_on == 0
    R_load_tb = (R_static + R_dynamic_tb) - Q0;
    [V_tb, ST_CDM] = V_TaylorBasis_pre(X,T,G,D,R_load_tb,U,nno,nel,nd,N_phi,zeta,dt_tb,t_CDM_tb,ST0,DST0,phi_given,Omega_given);
    V_tb_mid = V_tb(11*3-1,:);
elseif Taylor_on == 1 && DynamicOnly_on == 1
    R_load_tb = R_dynamic_tb;
    V_tb = V_TaylorBasis(X,T,G,D,R_load_tb,U,nno,nel,nd,N_phi,zeta,dt_tb,t_CDM_tb,ST0,DST0);
    V_tb_mid = V_tb(11*3-1,:);
elseif Taylor_on == 0
    V_tb_mid = zeros(1,length(t_CDM_tb));
end
