...
for i = 1:nel
  if ismember(i,Tflat) 
    G(i,:) = [E_deck A_deck I_deck rho c_damp];
  elseif ismember(i,Tincleft1)
    G(i,:) = [E_cable A_cable I_cable rho c_damp];
  ...
  elseif ismember(i,Tl)
    G(i,:) = [E_hanger A_hanger I_hanger rho c_damp];
...
% elements
[V0, ~, ~, ~, ~, R0_storage] = NewmarkStatic_R0(X,T,D,G,U,R_static,nno,nel,nd);
plotDof(X,T,D,V0,nel,5)

%Finding the vertical displacement of the deck corresponding to the places
%where the hangers connect
disp(V0((Numb/4+1)*3-1))
disp(V0((Numb/2+1)*3-1))
disp(V0((3*Numb/4+1)*3-1))
