...
% Mode parameters
N_phi = 3; % Number of linear modes included in the analysis
MD_no = 0;
...
% k0_phi1 = eVdyn'*K0ff*eVdyn;
% SS = diag(1./diag(sqrt(k0_phi1)));
% phi0 = eVdyn*SS; % Normalizing with respect to K0-matrix
% Phi0 = zeros(nd,nd);
% Phi0(df,df) = phi0; % setup a matrix containing the full mode-shapes normalized with respect to K-matrix
...
m_phiff = m_phi(df,df); % defining only the matrices for the free dofs
c_phiff = c_phi(df,df);
k0_phiff = k0_phi(df,df);
...
if linear_phi == 1
  ...
  % Second mode
  plotDof(X,T,D,Phi(:,4),nel,Vskala)            % plot deformed frame

  % Third mode
  plotDof(X,T,D,Phi(:,5),nel,Vskala)            % plot deformed frame
end
...
for i = 1:N_phi
  for j = 1:i
    ...
    alpha0 = phi(df,j)'/m_phiff(j,j)*(dKsi(df,df)*phi(df,j) - dKsj(df,df)*phi(df,i));
...
% PHI = [Phi(:,3:5) dPhi(:,1:end)];
% PHi = [phi(df,1:3) dphi(:,1:end)];
% PHi_lincomb = [phi(df,1:3) dphi_lincomb(:,1:end)];
...
Q_CDM_lin = zeros(size(V0,1),N+1);
Q0 = Q(X,T,G,nel,nno,V0);
DDV0 = Mff\(Rf_CDM(:,1) - Cff*DV0(df) - Q0(df));
DDS0 = (PHi'*Mff*PHi)\(Rphi_CDM(:,1) - PHi'*Cff*PHi*DS0 - PHi'*Q0(df));
DDV0_lin = Mff\(Rf_CDM(:,1) - Cff*DV0(df) - K0ff*V0(df));
V_before = V0(df) - dt*DV0(df) + (1/2)*dt^2*DDV0;
S_before = S0 - dt*DS0 + (1/2)*dt^2*DDS0; 
V_before_lin = V0(df) - dt*DV0(df) + (1/2)*dt^2*DDV0_lin;
...
V_CDM_Full_lin = zeros(nd,N+1);
V_CDM_lin = [V_before_lin, V0(df), zeros(size(V0(df),1),N-1)];
V_CDM_Full_lin(df,:) = V_CDM_lin;

a_CDM_phi = (1/dt^2)*PHi'*Mff*PHi - (1/(2*dt))*PHi'*Cff*PHi;
b_CDM_phi = (2/dt^2)*PHi'*Mff*PHi;
Z_CDM_phi = (1/dt^2)*PHi'*Mff*PHi + (1/(2*dt))*PHi'*Cff*PHi;
a_CDM = (1/dt^2)*Mff - (1/(2*dt))*Cff;
b_CDM = (2/dt^2)*Mff;
Z_CDM = (1/dt^2)*Mff + (1/(2*dt))*Cff;
...
%% Taylor relationshipt between modal coordinates
%dphi_11 = 1/6*dphi_32 - 1/4*dphi_22
...
u_plot = V_CDM_Full(11*3-1,2:end);%/max(V_CDM(end-1,2:end));
us_plot = Vs_CDM_Full(11*3-1,2:end);
u_plot_lin = V_CDM_Full_lin(11*3-1,2:end);

if V_plot == 1

  figure()
  plot(t_CDM,u_plot*1e2,':k.')
  hold on
  plot(t_CDM,us_plot*1e2,'--k')
  xlabel('time, t [s]')
  ylabel('Displacement, u [cm]')
  title('Displacement over time using modal basis projection')
  legend({'CDM non-linear', 'u_{non},CDM modal projection, u_{MD}'},'Location','northeast')
  hold off
end
