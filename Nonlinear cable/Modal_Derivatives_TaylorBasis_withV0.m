...
elseif V0_on == 1
    ...
    for i = 1:N_phi
      for j = 1:i
        dKsi = dKs_V0(X,T,G,nel,nno,phi(:,i),V0);
        dKsj = dKs_V0(X,T,G,nel,nno,phi(:,j),V0);
        alpha0 = phi(df,j)'/m_phi(j,j)*(dKsi(df,df)*phi(df,j) - dKsj(df,df)*phi(df,i));
        dphi = (K0ff - (Omega(i)+Omega(j))^2*Mff)\((alpha0*Mff - dKsi(df,df))*phi(df,j)*2);
        if i==j
          dphi = dphi/2;
...
%% Plots
% matFile = 'PhysicalandStandardBasis_withV0_ark1';
% matData = load(matFile);
% u_nonlinear = matData.unon_plot;
matFile = 'PhysicalandStandardBasis_withV0_Fullload_Ark1';
matData = load(matFile);
u_nonlinear = matData.unon_plot;
