...
c_damp = 1; % damping coefficient of the element
...
Mff_phi_Full = PHi_phys'*Mff*PHi_phys;
Cff_phi_Full = zeros(size(Mff_phi_Full,1),size(Mff_phi_Full,2));
zeta = 0.20;
if c_damp > 0
  for i = 1:size(phi,2)
      Cff_phi_Full(i,i) = 2*zeta*m_phi(i,i)*Omega(i);
  end
end

Cff_phys = (Mff*PHi_phys/m_phi)*Cff_phi_Full*(m_phi\PHi_phys'*Mff);
...
u_plot = V_CDM_Full(11*3-1,2:end);
...
if V_plot == 1
  ...
  title('Displacement over time using physical coordinates')
  legend('CDM physical, u','Location','northeast')
  hold off
end
