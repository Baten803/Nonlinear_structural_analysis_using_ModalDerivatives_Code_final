...
DT_CDM_s = zeros(11,1);
omega_dphi_s = zeros(11,1);

for x = 0:10
  E = 210e9*(1 - 0.0995*x); % [Pa] Young's modulus
  %rho = 7800*A*(1 - 0.0995*x);
  %N0 = 5e7*(1 + 0.5*x);
  ...
  R0 = R(X,T,D,dL,bL,nd,nel,N0); % Load amplitude vector
  K0 = Kt(X,T,G,D,nel,nno,V); % standard linear stiffness matrix
...
%% Stability of modal
  ...
  %------------- Set up -------------------
    % Matrices
    Mff_phi = PHi'*Mff*PHi;
    Mff_dphi = dPhi(df,:)'*Mff*dPhi(df,:);
    K0ff_dphi = dPhi(df,:)'*K0ff*dPhi(df,:);
    ...
    Cff_fysk = (Mff*PHi/Mff_phi)*Cff_phi*(Mff_phi\PHi'*Mff);

    % setup freq associated with the MD's
    omega_dphi = zeros(H,1);
    for y = 1:H
      omega_dphi(y) = sqrt(K0ff_dphi(y,y)/Mff_dphi(y,y));
    end

    % Initialize iteration steps
    dt_s = 2/max(omega_dphi);
    t_CDM_s = 0:dt_s:10;
    N_s = length(t_CDM_s);
    shrink = 0.95; % Reduction factor
    dt_min = 1e-6/max(Omega(1:N_phi));

   %--------------- Modal basis iteration process ----------------
  ...
  while true
    ...
    % Load vector
    R_CDM_s = R0*(F1*sin(w1*t_CDM_s) + F2*sin(w2*t_CDM_s)); % Load vector som er N+1 bred
    Rf_CDM_s = R_CDM_s(df,:);
    Rphi_CDM = PHi'*Rf_CDM_s;
    
    % setup initial conditions
    V0 = zeros(size(PHI,1),1); %Physical initial displacement of the system
    S0 = zeros(size(PHi,2),1); %Modal initial displacement of the system
    DV0 = zeros(size(PHI,1),1); %Physical initial velocity of the system
    DS0 = zeros(size(PHi,2),1); %Modal initial velocity of the system
    Q0 = Q(X,T,G,D,nel,nno,V0); %Initial internal restoring force of the system
    DDV0 = Mff\(Rf_CDM_s(:,1) - Cff_fysk*DV0(df) - Q0(df)); %Physical Initial acceleration of the system
    DDS0 = Mff_phi\(Rphi_CDM(:,1) - Cff_phi*DS0 - PHi'*Q0(df));%Modal initial acceleration of the system
    V_before = V0(df) - dt_s*DV0(df) + (1/2)*dt_s^2*DDV0; %Physical approximated displacement before initial position
    S_before = S0 - dt_s*DS0 + (1/2)*dt_s^2*DDS0; %Modal approximated displacement before initial position
    ...
    % dV_CDM_Full = zeros(nd,N);
    % dV_CDM_Full(:,1) = DV0;
    Vs_CDM_Full = zeros(nd,N_s+1);
    Vs_CDM = [V_before, V0(df), zeros(size(V0(df),1),N_s-1)];
    Vs_CDM_Full(df,:) = Vs_CDM;
    dVs_CDM_Full = zeros(nd,N_s);
    dVs_CDM_Full(:,1) = DV0;
    S_CDM = [S_before, S0, zeros(size(PHi,2),N_s-1)];
    
    a_CDM_phi = (1/dt_s^2)*Mff_phi - (1/(2*dt_s))*Cff_phi;
    b_CDM_phi = (2/dt_s^2)*Mff_phi;
    Z_CDM_phi = (1/dt_s^2)*Mff_phi + (1/(2*dt_s))*Cff_phi;
    dZ_CDM_phi = decomposition(Z_CDM_phi,'lu');
    
    % This calculates the nonlinear and linear physical displacement of the
    % structure as well as the physical displacements when using modal basis
    % without grouping as in the taylor basis
    for i = 2:N_s 
    
      Qsi = Q(X,T,G,D,nel,nno,Vs_CDM_Full(:,i));
      Qsi = Qsi(df);
    
      S_CDM(:,i+1) = dZ_CDM_phi\(Rphi_CDM(:,i-1) - PHi'*Qsi + b_CDM_phi*S_CDM(:,i) - a_CDM_phi*S_CDM(:,i-1));
      Vs_CDM_Full(df,i+1) = PHi*S_CDM(:,i+1);
      dVs_CDM_Full(:,i) = (1/(2*dt_s))*(Vs_CDM_Full(:,i+1) - Vs_CDM_Full(:,i-1));
    end

    unstable = any(~isfinite(Vs_CDM_Full),'all') || max(abs(Vs_CDM_Full(:))) > 1e6;
    ...
    if dt_s < dt_min
      warning('dt_s became smaller than the limit')
      break
    end
  ...
  ix = x+1;
  DT_CDM_s(ix) = dt_s;
  omega_dphi_s(ix) = max(omega_dphi);
...
DT_theoretical_s = zeros(11,1);
for x = 1:11
  DT_theoretical_s(x) = 2/omega_dphi_s(x);
end

%% Numerical vs linear stability time steps

DT_error_s = zeros(11,1);
DT_diff_s = DT_theoretical_s - DT_CDM_s;
for i = 1:11
  DT_error_s(i) = DT_diff_s(i)/DT_theoretical_s(i);
end
...
figure(1)
loglog(omega_dphi_s, DT_CDM_s*1e3, 'x', 'LineStyle','none','MarkerSize',8,'LineWidth',1.5);
hold on
loglog(omega_dphi_s, DT_theoretical_s*1e3,'-', 'LineWidth',2);
grid on
...
figure(2)
loglog(omega_dphi_s*(1/(2*pi)), DT_CDM_s*1e3, 'x', 'LineStyle','none','MarkerSize',8,'LineWidth',1.5);
hold on
loglog(omega_dphi_s*(1/(2*pi)), DT_theoretical_s*1e3,'-', 'LineWidth',2);
grid on
...
function R = R(X,T,D,dL,bL,nd,nel,N0)
