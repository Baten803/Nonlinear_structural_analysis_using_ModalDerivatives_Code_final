...
% Mode parameters
N_phi = 9; % Number of linear modes to use
...
% Solving the linearized eigenvalue problem for V = 0
...
% Solving the linearized eigenvalue problem for V = V0
...
phi_V0(df,:) = eVdyn_V0; % Here we have added zeros to the non-free dofs such that
                 % we have the full modeshape
Phi_V0 = zeros(nd,nd);
Phi_V0(:,df) = phi_V0; % full modal matrix with the zero modal vectors due to boundary conditions
                 % These are normalized with respect to the M-matrix we want it to be the K0-matrix   
...
for i = 1:N_phi
  for j = 1:i
    ...
    dphi_V0 = (K0ff_V0 - (Omega_V0(i)+Omega_V0(j))^2*Mff)\((alpha0*Mff - dKsi(df,df))*phi_V0(df,j)*2);
    if i==j
      dphi_V0 = dphi_V0/2;
    end
    ...
    dPhi_V0(df,k) = dphi_V0;  
  end
end


H_phys = size(phi,2)*(size(phi,2)+1)/2;
dPhi_phys = zeros(nd,H_phys);
...
for i = 1:size(phi,2)
  for j = 1:i
    ...
    dKsj = dKs_V0(X,T,G,nel,nno,phi_V0(:,j),V0);
    alpha0 = phi_V0(df,j)'/m_phi_V0(j,j)*(dKsi(df,df)*phi_V0(df,j) - dKsj(df,df)*phi_V0(df,i));
    dphi_phys = (K0ff_V0 - (Omega_V0(i)+Omega_V0(j))^2*Mff)\((alpha0*Mff - dKsi(df,df))*phi_V0(df,j)*2);
    if i==j
...
PHI_phys = [phi_V0 dPhi_phys];
PHi_phys = [phi_V0(df,:) dPhi_phys(df,:)];
...
PHI = [phi_V0(:,1:N_phi) dPhi_V0(:,1:end)]; % Phi-matrix with zeros for the nonfree dofs
PHi = [phi_V0(df,1:N_phi) dPhi_V0(df,1:end)];% phi-matrix only containing the free dofs
...
% Load vector
R_dynamic = R0_dynamic*(F1*sin(w1*t_CDM)+F2*sin(w2*t_CDM));
...
V_phys = V_physical_pre(X,T,G,D,R_CDM,U,nno,nel,nd,dt,t_CDM,V0,DV0,zeta,PHI_phys,size(phi,2),Omega_V0);
