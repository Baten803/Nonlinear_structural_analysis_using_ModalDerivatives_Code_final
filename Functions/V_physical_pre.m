function V_phys = V_physical_pre(X,T,G,D,R_CDM,U,nno,nel,nd,dt,t_CDM,V0,DV0,zeta,PHI,N_phi,Omega)
...
if zeta > 0
  for i = 1:N_phi
    Cff_phi(i,i) = 2*zeta*Mff_phi(i,i)*Omega(i);
  end
  Cff_phys = (Mff*PHi/Mff_phi)*Cff_phi*(Mff_phi\PHi'*Mff);
elseif zeta == 0
    C_damp = C(X,T,G,D,nd,nel);
    Cff_phys = C_damp(df,df);
end
