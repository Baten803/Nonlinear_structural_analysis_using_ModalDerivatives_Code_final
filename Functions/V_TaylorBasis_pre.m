function [V_tb, ST_CDM] = V_TaylorBasis_pre(X,T,G,D,R_CDM,U,nno,nel,nd,N_phi,zeta,dt,t_CDM,ST0,DST0,phi_given,Omega_given)
...
elseif norm(phi_given) > 0

    m_phi = phi_given'*M_mass*phi_given;
...
elseif norm(phi_given) > 0
    ...
    for i = 1:N_phi
      for j = 1:i
        dKsi = dKs(X,T,G,nel,nno,phi_given(:,i),V);
        dKsj = dKs(X,T,G,nel,nno,phi_given(:,j),V);
        alpha0 = phi_given(df,j)'/m_phi(j,j)*(dKsi(df,df)*phi_given(df,j) - dKsj(df,df)*phi_given(df,i));
        dphi = (K0ff - (Omega_given(i)+Omega_given(j))^2*Mff)\((alpha0*Mff - dKsi(df,df))*phi_given(df,j)*2);
        if i==j
          dphi = dphi/2;
        end
...
%% setting up the modal basis matrix
...
elseif norm(phi_given) > 0
    PHI = [phi_given(:,1:N_phi) dPhi(:,1:end)];
    PHi = [phi_given(df,1:N_phi) dPhi(df,1:end)];
end
...
if zeta > 0 && norm(phi_given) == 0
  for i = 1:N_phi
    cT_red(i,i) = 2*zeta*m_phi(i,i)*Omega(i);
  end
elseif zeta > 0 && norm(phi_given) > 0
    for i = 1:N_phi
        cT_red(i,i) = 2*zeta*m_phi(i,i)*Omega_given(i);
    end
end
fT = PHi'*Rf_CDM;

% Set up initial internal force
Qs0 = Q(X,T,G,nel,nno,V0);
Qs0 = Qs0(df);
ghat0 = PHi'*Qs0;
Us0 = Ubeam(N_phi,s0);
gbar0 = Us0'*ghat0;
