function [V, s, RR, Rsup, NN, V_storage, s_storage,R_storage] = NewmarkStatic_taylor(X,T,D,G,U,bL,dL,nno,nel,nd,PHI,N_phi)
...
Vmax = 0.06; %Maximum allowed size of a displacement increment
...
for i = 1:nstep
  ...
  last_converged_step = i;

end

s_storage = zeros(N_phi,nstep);
s = zeros(N_phi,1);

tol_s = 1e-10;
nit_s = 30;

for i = 1:last_converged_step
  V_target = V_storage(:,i);
  ...
  while js < nit_s
    ...
    % Jacobian with respect to s
    Js = PHI*Ubeam(N_phi,s);

    % Calculate s increment
    ds = Js\Fs;
  ...
  s_storage(:,i) = s;
  fprintf('step = %d, iterations = %d, ||F_s|| = %.3e\n', i, js, Fsnorm);

end

% The s corresponding to the final displacement
s = s_storage(:,last_converged_step);
